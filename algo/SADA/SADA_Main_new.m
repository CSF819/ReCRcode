function E = SADA_Main_new( X, splitTries)
% INPUT:
% X     - Data matrix
%OUTPUT:
% E    - edges in the form [a,b,P] presents a->b, P is Pvalue of the edge
[n,d]=size(X);
...
%     fprintf('Split Begin, Variable size %d \n', nv);
    [idxA,idxB,idxCut]=SADA_Split_new(X, splitTries);
%     fprintf('Split Done, A size: %d, B Size: %d, Cut Size: %d \n', sum(idxA), sum(idxB),sum(idxCut));
    ...
    if sum(idxA)==0 || sum(idxB)==0
        ...
        idxA=idxA|idxCut;
        idxB=idxB|idxCut;
        EA=SADA_Main_new( X(:, idxA),splitTries);
        EB=SADA_Main_new( X(:, idxB),splitTries);
